...
def define_G(input_nc, output_nc, ngf, which_model_netG, norm='batch', use_dropout=False, init_type='normal', gpu_ids=[]):
    ...
    if which_model_netG == 'resnet_9blocks':
        ...
        netG = UNet(input_nc, output_nc, 128)
    else:
...
class UNet(nn.Module):
  def __init__(self, input_nc, output_nc, nf):
    ...
    layer_idx += 1
    name = 'layer%d' % layer_idx
    layer7 = blockUNet(nf*8, nf*8, name, transposed=False, bn=False, relu=False, dropout=False)
    # input is 2 x  2
    # layer_idx += 1
    # name = 'layer%d' % layer_idx
    ...
    # self.fc1 = torch.nn.Linear(nf*8, nf*4)
    # self.fc2 = torch.nn.Linear(nf*4, 128)
    # self.fc = torch.nn.Linear(nf*8, 128)
    # self.relu = nn.ReLU()
  ...
  def forward(self, x):
    ...
    # return dout1, self.Sigmoid(self.fc2(self.fc1(out7.squeeze())))
    # return dout1, self.relu(self.fc(out7.squeeze()))
    return dout1#, self.fc2(self.fc1(out7.squeeze()))

class UNetxx(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=False):
        super(UNetxx, self).__init__()
